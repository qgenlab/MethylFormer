...
def edit_methylation(meth_df_name, regions_df_name, output_path):
    meth_df = pd.read_csv(meth_df_name, sep="\t", header=None, usecols=[0, 1, 2, 3, 4, 7],
    ...
    for region_id, region_group in regions_df.groupby("region_id"):
        ...
        if in_region.shape[0] < 10:
            continue
        meth_df.loc[in_region.index, "region_id"] = region_id
    ...
    out_cols = ["chr", "start", "end", "coverage", "modified_meth", "meth_noise", "region_id", "tag"]
    meth_df[out_cols].to_csv(output_path, sep="\t", index=False, header=False)
...
def main(data=None, regions_file="tmp/training_regions.bed"):
    files = "tmp/filtered_095_new_destranded_*5mc.new.methyl1_filtered_1.CpG_diff_*.bed" 
