...
# diffs = [-75, -60, -45, -30, -15, 15, 30, 45, 60, 75]
...
for f in inputs:
    ...
    for diff in diffs:
        ...
        print(new_df)
        print(data)
        new_df.to_csv(os.path.splitext(os.path.basename(f))[0] + "_diff_" + str(diff)+"_.bed", index=False, header=False)



all_files = glob.glob("./new_destranded_*_5mc.new.methyl1_filtered_1.CpG_diff_*_.bed")

inputs = [f for f in all_files if re.search(r'CpG_diff_(-?5)_', f)]

# inputs = glob.glob("./*_diff_*.bed")
...
#    for chrom in chroms:
#        data_chr = data[data["chrom"] == chrom]
#        CpG_chr = CpG[CpG["chrom"] == chrom]
#        for _, region in CpG_chr.iterrows():
#            CpG_regions = data_chr[(region["chromStart"] <= data_chr["chromStart"] ) & (region["chromEnd"] >= data_chr["chromEnd"])]
#            All_regions.append(CpG_regions)
#    combined_df = pd.concat(All_regions, ignore_index=True)
#    combined_df.to_csv("./" + os.path.basename(f))
