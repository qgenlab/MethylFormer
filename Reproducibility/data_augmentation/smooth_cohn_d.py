...
def process_file(file_path, window_bp=200, sigma=100):
    ...
    df[["chr", "start", "end", "coverage", "meth_pct", "cohen_d", "region_id", "tag"]].to_csv(out_file, sep="\t", index=False, header=None)
    df.to_csv("case/"out_file, sep="\t", index=False)
    return out_file

def main():
    input_files = glob.glob("case/*__edited.bed")  
    window_bp = 1000
    sigma = 500
